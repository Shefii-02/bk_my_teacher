...
class StudentClassDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final start = event.startDateFor(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(
                  "${dateKey.toLocal().toIso8601String().split('T').first} • ${event.timeStart} - ${event.timeEnd}",
                ),
              ],
            ),
            const SizedBox(height: 8),
            ...
            Wrap(
              ...
              children: [
                ...
                _infoTile(
                  "Attendance",
                  event.attendanceRequired ? "Required" : "Optional",
                ),
                _infoTile(
                  "Duration",
                  event.duration != null
                ...
                if (event.courseId != null)
                  _infoTile("Course", event.courseId.toString()),
              ],
            ),
            ...
            if (event.meetingPassword != null)
              Text("Password: ${event.meetingPassword}"),
            const SizedBox(height: 12),
            Row(
              children: [
  ...
  _openAddEventSheet(BuildContext context) {
  ...
  Widget _infoTile(String title, String value) {
    return Chip(label: Text("$title: $value"));
  }
...
class PlaceholderScreenForJoin extends StatelessWidget {
  ...
  const PlaceholderScreenForJoin({
    super.key,
    required this.event,
    required this.dateKey,
  });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Join / Streaming")),
      body: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          children: [
            Text(
              "Here you'll handle routing by 'class_link' and 'class source' (zoom, gmeet, youtube, agora, etc.).",
            ),
            const SizedBox(height: 12),
